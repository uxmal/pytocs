...
namespace Pytocs.Core.TypeInference
{
        ...
        IExpVisitor<DataType>
    {
        ...
        {
            DataType fun = c.fn.Accept(this);
            List<DataType> dtPos = c.args.Select(a => a.defval.Accept(this)).ToList();
            Dictionary<string, DataType> hash = new Dictionary<string, DataType>();
            if (c.keywords != null)
            {
                foreach (Argument k in c.keywords)
                {
                    hash[k.name.Name] = k.defval.Accept(this);
                }
            }

            DataType dtKw = c.kwargs?.Accept(this);
            DataType dtStar = c.stargs?.Accept(this);
        ...
        {
            DataType it = f.variable.Accept(this);
        ...
        {
            List<DataType> elTypes = new List<DataType>();
            foreach (Exp el in list.Expressions)
            {
                DataType elt = el.Accept(this);
                elTypes.Add(elt);
            }
        ...
        {
            DataType it = unpacker.Iterable.Accept(this);
            IterableType iterType = analyzer.TypeFactory.CreateIterable(it);
            return iterType;
        }
        ...
        public DataType VisitLambda(Lambda lambda)
        {
            ...
            FunType fun = new FunType(lambda, env);
            fun.Table.Parent = scope;
        ...
        {
            DataType dt = VisitFunctionDef((FunctionDef)a.Statement, true);
            return dt;
        }
        ...
        public DataType VisitSuite(SuiteStatement b)
        {
            ...
            IEnumerable<Identifier> globalNames = b.stmts
                .OfType<GlobalStatement>()
                .SelectMany(g => g.names)
                .Concat(b.stmts
                    .OfType<NonlocalStatement>()
                    .SelectMany(g => g.names));
            foreach (Identifier id in globalNames)
            {
            ...
            bool returned = false;
            DataType retType = DataType.Unknown;
            foreach (Statement n in b.stmts)
            {
        ...
        public DataType VisitClass(ClassDef c)
        {
            ...
            if (c.body != null)
            {
                TypeTransformer xform = new TypeTransformer(classType.Table, analyzer);
                c.body.Accept(xform);
            }
            return DataType.Cont;
        }
        ...
        {
            foreach (Exp n in d.Expressions.AsList())
            {
                n.Accept(this);
        ...
        {
            foreach (AliasedName a in i.names)
            {
        ...
        public DataType VisitFrom(FromStatement i)
        {
            ...
            {
                foreach (AliasedName a in i.AliasedNames)
                {
        ...
        {
            foreach (WithItem item in w.items)
            {
        ...
        private void BindClassSlots(Exp eSlotNames)
        {
            ...
            else
            {
                ...
                foreach (Str slotName in slotNames.OfType<Str>())
                {
                    Identifier id = new Identifier(slotName.s, slotName.Filename, slotName.Start, slotName.End);
            ...
            DataType star)
        {
            ...
            if (fun is ClassType)
            {
                InstanceType instance = new InstanceType(fun);
            ...
            Exp call)
        {
            ...
            if (call != null && analyzer.InStack(call))
            {
            ...
            if (func.arrows.TryGetValue(fromType, out DataType cachedTo))
            {
                func.SelfType = null;
                return cachedTo;
            }
        ...
        public static DataType FirstArgumentType(FunctionDef f, FunType func, DataType selfType)
        {
            ...
            if (!func.Definition.IsStaticMethod())
            {
                if (func.Definition.IsClassMethod())
                {
                    ...
                    if (selfType != null && selfType is InstanceType inst)
                    {
        ...
        public DataType VisitFunctionDef(FunctionDef f, bool isAsync)
        {
            ...
            ClassType ct = scope.DataType as ClassType;
            if (ct != null)
            {
                fun.Class = ct;
            }
            ...
            DataType firstArgType = FirstArgumentType(f, fun, ct);
            if (firstArgType != null)
            {
                fun.Table.Bind(analyzer, f.parameters[0].Id, firstArgType, BindingKind.PARAMETER);
            }

            f.body.Accept(new TypeTransformer(fun.Table, analyzer));
            return DataType.Cont;
        }
        ...
        private void ImportStar(FromStatement i, DataType mt)
        {
            ...
            {
                foreach (string s in lt.values.OfType<string>())
                {
                    names.Add(s);
                }
            }
            ...
            else
            {
                ...
                foreach (KeyValuePair<string, ISet<Binding>> e in mt.Table.entrySet().Where(en => !en.Key.StartsWith("_")))
                {
        ...
        public ModuleType VisitModule(Module m)
        {
            ...
            if (m.body != null)
            {
                m.body.Accept(new TypeTransformer(mt.Table, analyzer));
            }
