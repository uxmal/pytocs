...
namespace Pytocs.Core.CodeModel
{
    public class CSharpExpressionWriter : ICodeExpressionVisitor
    {
        ...
        private static readonly Dictionary<CodeOperatorType, int> operatorPrecedence =
            new Dictionary<CodeOperatorType, int>
            {
        ...
        public void VisitArrayIndexer(CodeArrayIndexerExpression aref)
        {
            ...
            writer.Write("[");
            string sep = "";
            foreach (CodeExpression sub in aref.Indices)
            {
                writer.Write(sep);
                sep = ",";
        ...
        public void VisitApplication(CodeApplicationExpression app)
        {
            ...
            writer.Write("(");
            string sep = "";
            foreach (CodeExpression e in app.Arguments)
            {
                writer.Write(sep);
                sep = ", ";
        ...
        public void VisitArrayInitializer(CodeArrayCreateExpression arr)
        {
            ...
            else
            {
                ...
                ++writer.IndentLevel;
                string sep = ",";
                foreach (CodeExpression initializer in arr.Initializers)
                {
        ...
        {
            int prec = operatorPrecedence[bin.Operator];
            bool needParens =
                prec < precedence ||
                prec == precedence && parensIfSamePrecedence;
            if (needParens)
            {
                writer.Write("(");
            }
        ...
        {
            bool needParens = precedence > PrecUnary;
            if (needParens)
            {
                writer.Write("(");
            }
        ...
        public void VisitCollectionInitializer(CodeCollectionInitializer i)
        {
            ...
            bool sep = false;
            foreach (CodeExpression v in i.Values)
            {
                if (sep)
                {
                    writer.Write(",");
        ...
        public void VisitObjectInitializer(CodeObjectInitializer i)
        {
            ...
            bool sep = false;
            foreach (MemberDeclarator md in i.MemberDeclarators)
            {
                if (sep)
                {
                    writer.Write(",");
        ...
        public void VisitLambda(CodeLambdaExpression l)
        {
            ...
            else
            {
                writer.Write("(");
                string sep = "";
                foreach (CodeExpression arg in l.Arguments)
                {
                    writer.Write(sep);
                    sep = ",";
                    arg.Accept(this);
            ...
            {
                CSharpStatementWriter sw = new CSharpStatementWriter(writer);
        ...
        public void VisitMethodReference(CodeMethodReferenceExpression m)
        {
            ...
            if (m.TypeReferences.Count > 0)
            {
                writer.Write("<");
                string sep = "";
                foreach (CodeTypeReference tr in m.TypeReferences)
                {
                    writer.Write(sep);
                    sep = ", ";
        ...
        {
            bool needParens = precedence > PrecBase;
            if (needParens)
            {
                writer.Write("(");
            }
            ...
            ++writer.IndentLevel;
            foreach (CodeQueryClause clause in q.Clauses.Skip(1))
            {
                writer.WriteLine();
        ...
        public void VisitObjectCreation(CodeObjectCreateExpression c)
        {
            ...
            if (c.Arguments.Count > 0 || c.Initializers.Count == 0 && c.Initializer == null)
            {
                writer.Write("(");
                string sep = "";
                foreach (CodeExpression e in c.Arguments)
                {
                    writer.Write(sep);
                    sep = ", ";
                    e.Accept(this);
            ...
            if (c.Initializers.Count > 0)
            {
                ...
                ++writer.IndentLevel;
                string sep = null;
                foreach (CodeExpression e in c.Initializers)
                {
                    if (sep != null)
                    {
                        writer.Write(sep);
        ...
        public void VisitValueTuple(CodeValueTupleExpression tuple)
        {
            ...
            else
            {
                writer.Write("(");
                string sep = "";
                foreach (CodeExpression exp in tuple.Expressions)
                {
                    writer.Write(sep);
                    sep = ", ";
                    exp.Accept(this);
        ...
        {
            int oldPrec = precedence;
            bool oldParens = parensIfSamePrecedence;
            precedence = prec;
        ...
        {
            string dd = real.ToString(CultureInfo.InvariantCulture);
        ...
        private void WriteByteLiteral(Bytes literal)
        {
            writer.Write("new");
            writer.Write(" ");
            writer.Write("byte[]");
        ...
        private void WriteStringLiteral(Str literal)
        {
        ...
        public void VisitTypeReference(CodeTypeReference t)
        {
            ...
            if (t.TypeArguments.Count > 0)
            {
                writer.Write("<");
                string sep = "";
                foreach (CodeTypeReference ta in t.TypeArguments)
                {
                    writer.Write(sep);
                    sep = ", ";
        ...
        private void GenerateTypeName(string typeName)
        {
            ...
            {
                if (csharpTypenames.TryGetValue(typeName, out string csharpName))
                {
