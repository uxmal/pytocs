...
namespace Pytocs.UnitTests.TypeInference
{
    public class AnalyzerTests
    {
        public AnalyzerTests()
        {
            ...
            an = new AnalyzerImpl(fs, logger, options, DateTime.Now);
        }
        ...
        {
            string sActual = BindingsToString();
            if (sExp != sActual)
            {
                Console.WriteLine(sActual);
                string[] split = new[] { nl };
                StringSplitOptions opt = StringSplitOptions.None;
                string[] aExp = sExp.Split(split, opt);
                string[] aActual = sActual.Split(split, opt);
                int i;
        ...
        private string BindingsToString()
        {
            ...
            IEnumerator<Binding> e = an.GetAllBindings().Where(b => !b.IsBuiltin && !b.IsSynthetic).GetEnumerator();
            while (e.MoveNext())
            {
...
");
            ...
            string sExp =
                ...
                @"(binding:kind=MODULE:node=(module:\sys_q\parsing\parser.py):type=parser:qname=.sys_q.parsing.parser:refs=[])" +
                nl +
                @"(binding:kind=CLASS:node=Parser:type=<Parser>:qname=.sys_q.parsing.parser.Parser:refs=[Parser,Parser])" +
                nl +
                @"(binding:kind=METHOD:node=parse:type=(Parser, ?) -> None:qname=.sys_q.parsing.parser.Parser.parse:refs=[p.parse])" +
                nl +
                @"(binding:kind=PARAMETER:node=self:type=<Parser>:qname=.sys_q.parsing.parser.Parser.parse.self:refs=[])" +
                nl +
                ...
                @"(binding:kind=PARAMETER:node=self:type=Parser:qname=.sys_q.parsing.parser.Parser.parse.self:refs=[])" +
                nl +
                @"(binding:kind=PARAMETER:node=phile:type=?:qname=.sys_q.parsing.parser.Parser.parse.phile:refs=[])" +
                nl +
...
");
            ...
            string sExp =
                ...
                @"(binding:kind=PARAMETER:node=point:type=?:qname=.foo.test.bar.point:refs=[point,point,point,point])" +
                nl;
